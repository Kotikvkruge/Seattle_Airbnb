...
for (i in col_names_num){
  hist(dat_clean[[i]], main = i, breaks = 25)
}
par(mfrow=c(1,1))
...
# 
# lin_reg2 <- lm(price ~ room_type + reviews + overall_satisfaction + I(overall_satisfaction ^ 2) + accommodates
#            + bedrooms + bathrooms + distr + I(reviews^2) + dist_to_cent_norm, data = dat_clean)
# summary(lin_reg2)
# crPlots(lin_reg2)
# coeftest(lin_reg2, vcov = vcovHC(lin_reg2, type = "HC0"))
...
# q_reg_0.1 <- rq(price ~ room_type + reviews + overall_satisfaction
#                 + bedrooms + bathrooms +  longitude + latitude + no_reviws, 
#                 data = dat_clean, tau = 0.1)
# summary(q_reg_0.1)
# 
# q_reg_0.9 <- rq(price ~ room_type + reviews + overall_satisfaction
#                 + bedrooms + bathrooms +  longitude + latitude + no_reviws, 
#                 data = dat_clean, tau = 0.9)
# summary(q_reg_0.9)
...
# smoothQ_m <- rqss(price ~ qss(cbind(longitude,latitude), lambda = .8) + reviews
#                   + bedrooms + bathrooms + overall_satisfaction +  no_reviws +
#                     shared_room + private_room + entire_home
#                   , tau = 0.5, data = dat_clean)
...
upper_bond <- quant_coef[,c(3:7)] + 1.96 * quant_se[,c(3:7)]
lower_bond <- quant_coef[,c(3:7)] - 1.96 * quant_se[,c(3:7)]
...
grid.arrange(overall_satisfaction_pl,bathrooms_pl, bedrooms_pl, reviews_pl,ncol=2)
...
plot(price_predict_q_m_med_rast, main = 'Price surface of Airbnb in Seattle - QR')
plot(price_surf, main = 'Price surface of Airbnb in Seattle - SmoothLUR')
par(mfrow=c(1,1))

par(mfrow=c(1,3))
plot(price_predict_q_m_med_rast, main = 'Median Price surface of Airbnb in Seattle')
# plot(price_surf_q_0.1, main = 'Tau = 0.1 Price surface of Airbnb in Seattle')
# plot(price_surf_q_0.9, main = 'Tau = 0.9 Price surface of Airbnb in Seattle')
par(mfrow=c(1,1))

# 4.2
plot(x = nd$bedrooms, y = pred_q_m_med)

ggplot(mapping = aes(x = nd$bedrooms, y = pred_q_m_med)) +
  geom_point()+ 
  geom_smooth()
